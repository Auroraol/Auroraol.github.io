...
(function () {
  ...
  function initTocCollapseControls() {
    ...
    tocRoot.querySelectorAll('li').forEach((item, index) => {
      ...
      const childList = directChildByTag(item, 'UL') || directChildByTag(item, 'OL');
      if (!link || !childList) {
        return;
      }
      item.classList.add('toc-collapsible');
      if (!childList.id) {
        childList.id = "toc-sublist-".concat(index);
      }
      ...
      button.setAttribute('aria-label', "\u6298\u53E0 ".concat(link.textContent.trim()));
      button.addEventListener('click', event => {
        event.preventDefault();
        event.stopPropagation();
        const collapsed = item.classList.toggle('toc-collapsed');
        button.setAttribute('aria-expanded', String(!collapsed));
        button.setAttribute('aria-label', "".concat(collapsed ? '展开' : '折叠', " ").concat(link.textContent.trim()));
      });
