...
**Go 里函数传参永远是值传递。**所以不要说 Go 有引用传递。更准确的说法是：<span style="color:red">Go 都是值传递，只是有些值里面保存了指向底层数据的指针</span>>。
...
```
map 变量 -> hmap即哈希映射（Hash Map），是一种基于哈希表实现的关联数组 -> buckets 桶数组 -> bmap 桶 -> key/value
```

也就是：**map 本身不是直接存数据，而是指向一个 hmap，真正的数据在桶里**
...
map 是按 hash 放桶，不是按顺序存；扩容会搬 key；Go 还故意随机遍历起点，防止你依赖顺序。

### 3.4 Map如何实现顺序读取？

正确做法是：先取 key，再排序，再遍历。
