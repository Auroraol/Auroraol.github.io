...
来源: https://colobu.com/gotips/051.html

---

# Tip #51 避免使用init()
原始链接： Golang Tip #51: Avoid using init()`init()`是一个特殊的函数，它在主函数**之前**和全局变量初始化**之后**运行：
![](images/051/001.png)
它通常用于准备一些全局变量，但最好保持全局变量的数量尽量少。
[Golang Tip #48 避免使用全局变量，尤其是可变变量](./048.html)
我们的示例无法将`precomputedValue`设置为常量值，因为它依赖于在运行时计算的值`math.Sqrt(2)`。
但是，有一种更好的方法：
![](images/051/002.png)
“现在，为什么我们应该避免使用 init()？”- 副作用
