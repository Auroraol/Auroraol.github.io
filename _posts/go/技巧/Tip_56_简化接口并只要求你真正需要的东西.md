...
来源: https://colobu.com/gotips/056.html

---

# Tip #56 简化接口并只要求你真正需要的东西
原始链接：Golang Tip #56: Simplify interfaces and only ask for what you really need在Go中定义接口时，请遵循以下技巧：
[1.]()只有在实际需要时才定义接口。
[2.]()接受接口并返回具体类型。
[3.]()将接口放在它们被使用的地方（消费者），而不是它们被创建的地方（生产者）。([Tip #18](./018.html))
...
“为什么这对测试来说是不利的？”当我们进行测试时，我们不应该搞清楚输入接口的哪些方法被使用了，对吧？
