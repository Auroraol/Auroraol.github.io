...
来源: https://colobu.com/gotips/082.html

---

# Tip #82 使用%+v和%q来增强调试输出，而不是使用%v和%s
原始链接：Golang Tip #82: Enhance Debug Output with %+v and %q Instead of %v and %s当我们进行调试时，能够从日志中获得更清晰、更详细的输出对我们来说这会是一个巨大的帮助。
...
“%q 怎么样？”`%q`本质上是一个将字符串用引号括起来的快捷方式，而`%s`只是原样输出字符串。
这种区别在处理以下情况时特别有用：
- 可能是空的字符串
- 或者包含特殊字符或空格，这些字符不是立即可见的：
![](images/082/3.png)
只要看一下使用每种格式化选项生成的日志，就会注意到当使用`%q`时，问题变得多么清晰？
“但是有什么缺点呢？”不过，有一点需要注意，那就是`%q`不会解释像`	`或`
