...
- <span style="color:red">采用驼峰命名方式，首字母根据访问权限, 来决定使用大写或小写(包括结构体名称、属性、方法)</span>
- 结构体名不应该是动词，应该是名词
...
<span style="color:red">工作中一般使用 u:= &User{name: "yangyongjie",...} 这种方式实例化并初始化</span>
...
| 场景           | 是否零值初始化   | 示例                        |
| -------------- | ---------------- | --------------------------- |
| `var p *T`     | ❌ 否（指针 nil） | `var p *Server` // p 是 nil |
| ` s := &T{}`   | ✅ 是             | `s := &Server{}`            |
| `var s T`      | ✅ 是             | `var s Server`              |
| `s := T{}`     | ✅ 是（显式零值） | `s := Server{}`             |
| `new(T)`       | ✅ 是             | `new(Server)`               |
| `&T{}`         | ✅ 是             | `&Server{}`                 |
| `make([]T, n)` | ✅ 是（每个元素） | `make([]Server, 5)`         |
| 类型断言失败   | ✅ 是（返回零值） | `v, ok := i.(Server)`       |
...
<span style="color:red">结构体变量名.成员名,  注: &结构体名.成员名 不能访问</span>

<span style="color:green">当前结构体可以直接访问其内嵌结构体的内部字段</span>
...
+ 结构体指针指向的是结构体变量的内存地址(使用 整体赋值`*rsp = *pteRsp` ,   属性赋值rsp .xxx =pteRsp.xxx  
...
容易误改**。Go 这里的规则就是：**值传递只保护“外壳”，不一定保护“里面指向的数据”。
...
普通字段：改副本，不影响外面
指针字段：改指向的对象，会影响外面
map 字段：改 map 内容，会影响外面
slice 字段：改元素，会影响外面
slice append：不一定影响，要看是否扩容
```

**传结构体不是指针，只能保证最外层结构体是副本；里面如果有指针、map、slice，底层数据还是可能被改。**
...
type Event struct {
    ...
    Body      Body              // 普通结构体字段
    BodyPtr   *Body               // 指针字段
    Ctx       map[string]string     // map 字段
    Receivers []User              // slice 字段
}

func change(req Event) {
    // 1. 改普通字段：不会影响外面
    req.Name = "new-name"

    // 2. 改普通结构体字段：不会影响外面
    req.Body.Nick = "new-body"

    // 3. 改指针指向的对象：会影响外面
    req.BodyPtr.Nick = "new-body-ptr"

    // 4. 改 map 内容：会影响外面
    req.Ctx["trace_id"] = "new-trace"

    // 5. 改 slice 里的元素：会影响外面
    req.Receivers[0].Nick = "new-user"

    // 6. append 改的是 req.Receivers 这个 slice 头副本
    // 外面的 Receivers 长度不会变
    req.Receivers = append(req.Receivers, User{Nick: "append-user"})
...
func main() {
    event := Event{
        Name:    "old-name",
        Body:    Body{Nick: "old-body"},
        BodyPtr: &Body{Nick: "old-body-ptr"},
...
outside: {
	Name: old-name,
	Body: {Nick: old-body},
	BodyPtr: {Nick: new-body-ptr},
	Ctx: map[trace_id:new-trace],
	Receivers: [{Nick:new-user}]
}
```
...
Ctx["trace_id"] 变了
Receivers[0].Nick 变了
Receivers 长度没变，没有 append-user
```

原因就是：

```
req 是副本，所以改 req.Name、req.Body 不影响外面。

但 req.BodyPtr、req.Ctx、req.Receivers 里面都带着指向底层数据的地址。

所以通过这些地址去改底层内容，会影响外面。
```

如果你想完全不影响外面，就要深拷贝：

```go
func safeChange(req Event) {
    req.BodyPtr = &Body{Nick: req.BodyPtr.Nick}

    req.Ctx = map[string]string{}
        for k, v := range req.Ctx {
        req.Ctx[k] = v
    }

    req.Receivers = append([]User(nil), req.Receivers...)
