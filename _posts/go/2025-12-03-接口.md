...
+ 示例
  ...
  type Order struct {
  	BaseModel        // 匿名嵌入：直接"拥有" BaseModel 的所有方法
  	Logger           // 再来一个组件
  	Customer string
  }
  ...
  ```
  // ✅ 推荐：匿名嵌入用于真正的"is-a"关系
  type Model struct {
      BaseModel           // Model 是一个 BaseModel
      Logger              // Model 有日志能力
  }
  
  // ✅ 推荐：具名嵌入用于"has-a"关系或工具类
  type Service struct {
      db     *sql.DB     // 依赖，不是继承
      cache  *RedisClient // 另一个依赖
      logger Logger       // 具名，避免冲突
  }
  ```
  
  1. 匿名嵌入（Anonymous Embedding）—— 最常用
  ...
  u.String()  // 直接调用 BaseModel 的方法
  u.Log("created")  // 直接调用 Logger 的方法
  fmt.Println(u.ID) // 直接访问 BaseModel 的字段
  ```
  
  2. 具名嵌入（Named Embedding）—— 显式访问
  
  如果你不想方法/字段被自动提升，或者需要嵌入同类型的多个实例：
  
  ```go
  type User struct {
  	Base BaseModel  // 具名嵌入：不会提升方法
  	Log  Logger     // 需要显式访问
  	Name string
  }
  ...
  // 使用
  u := User{
      Base: BaseModel{ID: 1, CreatedAt: time.Now()},
      Log:  Logger{Prefix: "USER"},
      Name: "Alice",
  }
  
  // 必须显式访问
  fmt.Println(u.Base.ID)       // ✅ 正确
  u.Base.String()            // ✅ 必须这样调用
  // u.String()              // ❌ 编译错误：User 没有 String 方法
  // u.ID                  // ❌ 编译错误
  ```
