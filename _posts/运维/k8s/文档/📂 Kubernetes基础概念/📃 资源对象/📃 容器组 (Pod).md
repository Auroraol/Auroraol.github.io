...
<img src="https://cdn.nlark.com/yuque/0/2020/png/2213540/1597324782310-05d9bf59-ff37-4312-8585-bd882bc76285.png#align=left&display=inline&height=400&originHeight=400&originWidth=600&size=0&status=done&style=none&width=600" style="zoom: 67%;" />
...
实际应用场景中，很少会直接在 kubernetes 中创建单个 Pod。在 Kubernetes 中通常是使用 Controller 来管理 Pod 的。

因为 Pod 的生命周期是短暂的，用后即焚的实体。当 Pod 被创建后（不论是由你直接创建还是被其他 Controller），都会被 Kubernetes 调度到集群的 Node 上。直到 Pod 的进程终止、被删掉、因为缺少资源而被驱逐、或者 Node 故障之前这个 Pod 都会一直保持在那个 Node 上。
...
1. 如果 Pod 运行的 Node 故障，或者是调度器本身故障，这个 Pod 就会被删除。

2. 同样的，如果 Pod 所在 Node 缺少资源或者 Pod 处于维护状态，Pod 也会被驱逐。Kubernetes 使用更高级的称为 Controller 的抽象层，来管理 Pod 实例。
...
![image-20260328170141269](C:/Users/16658/AppData/Roaming/Typora/typora-user-images/image-20260328170141269.png)
...
| 状态                  | 说明                                 | 处理建议                                  |
| --------------------- | ------------------------------------ | ----------------------------------------- |
| **Pending**           | Pod 已创建但未被调度，或正在拉取镜像 | 检查资源限制、节点容量、镜像拉取进度      |
| **ContainerCreating** | 正在创建容器                         | 通常短暂状态，长时间停留需检查存储挂载    |
| **Error**             | 容器启动失败                         | 查看 `kubectl logs` 和 `kubectl describe` |
| **CrashLoopBackOff**  | 容器反复崩溃重启                     | 检查应用日志、启动命令、依赖服务          |
| **OOMKilled**         | 内存不足被系统杀死                   | 增加内存限制或优化应用内存使用            |
| **Evicted**           | 节点资源不足，Pod 被驱逐             | 检查节点磁盘/内存压力，清理资源或扩容     |
| **Completed**         | 容器正常执行完毕（Job类）            | 正常状态，无需处理                        |
| **Terminating**       | Pod 正在被删除                       | 若长时间卡住，检查 finalizers 或强制删除  |
